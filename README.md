# Blockchain Commons MUR for Rust
...
`bc-mur` is a multipart UR QR code generator that produces single-frame and animated fountain-coded QR sequences with optional logo overlay.
...
The `mur` command-line tool supports three subcommands:

- **`mur single`** — Render a single QR code image (PNG)
- **`mur animate`** — Generate an animated multipart fountain-coded QR sequence (GIF or ProRes)
- **`mur frames`** — Dump individual frames as numbered PNGs

## Version History

- **0.1.0, March 16, 2026**
  - Initial release: multipart UR QR code generator.
  - Single-frame QR code rendering from raw bytes or UR strings.
  - Logo overlay with module-aligned compositing (square or circle clear shape).
  - Animated multipart fountain-coded QR sequences (GIF output).
  - ProRes 4444 encoding via optional ffmpeg integration.
